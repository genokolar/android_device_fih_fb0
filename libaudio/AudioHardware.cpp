...
static const uint32_t DEVICE_HEADSET_MONO_RX = 3;      /* headset_mono_rx */
...
static const uint32_t DEVICE_SPEAKER_RX = 6;           /* speaker_mono_rx */
...
void printTable() {
    ...
    temp_ptr = head->next;
    while (temp_ptr != NULL) {
        printf("%d %d %d %d %d\n", temp_ptr->dec_id, temp_ptr->dev_id, temp_ptr->dev_id_tx,
                                   temp_ptr->stream_type, temp_ptr->active);
        temp_ptr = temp_ptr->next;
    }
}
...
static status_t set_volume_rpc(uint32_t device,
                               uint32_t method,
                               uint32_t volume) {
    LOGV("set_volume_rpc(%d, %d, %d)", device, method, volume);
...
status_t AudioHardware::setVoiceVolume(float v) {
    ...
    if (mCurSndDevice == SND_DEVICE_HAC) {
        ...
        LOGI("Setting in-call volume to %d (available range is 0 to %d)", vol, VOICE_VOLUME_MAX);
...
static status_t do_route_audio_rpc(uint32_t device,
    ...
    switch (device) {
        ...
        case SND_DEVICE_IN_S_SADC_OUT_HANDSET:
            ...
            break;
        case SND_DEVICE_IN_S_SADC_OUT_SPEAKER_PHONE:
            new_rx_device = DEVICE_SPEAKER_RX;
...
status_t AudioHardware::enableFM(uint32_t sndDevice) {
    LOGD("enableFM");
...
status_t AudioHardware::AudioSessionOutMSM7xxx::set(AudioHardware* hw,
                                     uint32_t devices, int *pFormat, int32_t sessionId) {
...
status_t AudioHardware::AudioSessionOutMSM7xxx::setVolume(float left, float right) {
    ...
    if (v < 0.0) {
        LOGW("AudioSessionOutMSM7xxx::setVolume(%f) under 0.0, assuming 0.0", v);
        v = 0.0;
    } else if (v > 1.0) {
        LOGW("AudioSessionOutMSM7xxx::setVolume(%f) over 1.0, assuming 1.0", v);
        v = 1.0;
    }

    /* Ensure to convert the log volume back to linear for LPA */
    float vol = v * 100;
    LOGV("AudioSessionOutMSM7xxx::setVolume(%f)", v);
    LOGV("Setting session volume to %f (available range is 0 to 100)", vol);
...
ssize_t AudioHardware::AudioStreamOutMSM72xx::write(const void* buffer, size_t bytes) {
    ...
    unsigned short dec_id = (unsigned short)INVALID_DEVICE;
...
SIDE EFFECTS
    None.
===========================================================================*/
int amrsup_frame_len_bits(amrsup_frame_type frame_type,
...
SIDE EFFECTS
    None.
===========================================================================*/
int amrsup_frame_len(amrsup_frame_type frame_type, amrsup_mode_type amr_mode) {
...
static int amrsup_if1_framing(
    ...
    memset(if1_frame, 0, amrsup_frame_len(AMRSUP_SPEECH_GOOD, AMRSUP_MODE_1220));
    ...
    switch (frame_type) {
        ...
        case AMRSUP_SID_UPDATE:
            ...
            if (frame_type == AMRSUP_SID_UPDATE)
                ...
                if1_frame[4] |= (((unsigned char)amr_mode << 3) & 0x08) |
                                (((unsigned char)amr_mode << 1) & 0x04) |
                                (((unsigned char)amr_mode >> 1) & 0x02);
        ...
        case AMRSUP_SPEECH_GOOD:
            /* Set frame type index */
            if1_frame_info->frame_type_index = (amrsup_frame_type_index_type)(amr_mode);
